...
MODEL_PATH = SAVE_DIR / "lr_regressor_final.joblib"
...
def main():
    ...
    # --------------------------------------------------------
    # 2) MODEL VE PIPELINE (Linear Regression)
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Linear Regression basit bir modeldir ama yine de 
    # sabit terim (intercept) kullanıp kullanmayacağını optimize edebiliriz.
    param_grid = {
        "reg__fit_intercept": [True, False],
        "reg__positive": [True, False], # Katsayıların sadece pozitif olmasını zorlar
    }

    # --------------------------------------------------------
    # 4) OPTİMİZASYON: 10-FOLD GRID SEARCH
    # --------------------------------------------------------
    print("\n=== LINEAR REGRESSION — 10-FOLD GRID SEARCH BAŞLIYOR ===")
    ...
    print("\n=== NİHAİ MODEL PERFORMANSI (LR - 10-Fold CV) ===")
    print(f"MAE      : {mae:.3f}")
    ...
    plt.title(f"Linear Regression (10-Fold CV)\nR2={r2:.3f} | Adj.R2={adj_r2:.3f} | RMSE={rmse:.3f}")
    plt.legend()
    plt.grid(True, linestyle=":", alpha=0.6)
    plt.tight_layout()
    plt.savefig(SAVE_DIR / f"lr_scatter_{current_time}.png", dpi=400)
    plt.show()
    ...
    plt.grid(True, linestyle=":", alpha=0.6)
    plt.tight_layout()
    plt.savefig(SAVE_DIR / f"lr_residuals_{current_time}.png", dpi=400)
    plt.show()
